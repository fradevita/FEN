program ellipse
    ...
    type(grid)                        :: comp_grid
    type(bc_type)                     :: bc(4)
    type(lagrangian_solid_1D), target :: S
    ...
    S%rotation_center => S%center_of_mass%X
    ...
    allocate(S%IM(3))
    S%IM(1:2) = rhos*pi*a*b*0.25_dp
    S%IM(3) = 0.2_dp*S%IM(1)*((a/2.0_dp)**2 + (b/2.0)**2)
    ...
    if (myrank == 0) write(out_id, '(A14)') 't,x,y,thetaz,v'
    ...
    time_loop: do while(time < 22.5_dp)
        ...
        if (mod(step,100) == 0) then
            if (myrank == 0) write(out_id,'(*(E16.8,:,","))') time, S%center_of_mass%X(1:3), &
                                                                    S%center_of_mass%V(2)
        endif
    end do time_loop
