...
mpirun -n 1 ./run.e 1 > run_log 2> run_warnings
mpirun -n 1 ./run.e 2 > run_log 2> run_warnings
...
bash clean.sh > /dev/null
make SOURCE=test_FSI_E.f90 ADDFLAGS='-DIBM -DFSI' > compilation_log 2> compilation_warnings
mpirun -n 1 ./run.e 1 > run_log 2> run_warnings
mpirun -n 1 ./run.e 2 > run_log 2> run_warnings
diff data/state_0000010.raw data/state_0000020.raw > diff_out
NL=$(wc diff_out | awk '{print $1}')
