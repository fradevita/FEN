program ABC
    ...
    use global_mod    , only : pi, myrank, ierror
    use grid_mod 
    use solver_mod
    use navier_stokes_mod , only : viscosity, set_timestep, v, p
    use IO_mod
        ...
        do k = comp_grid%lo(3),comp_grid%hi(3)
            do j = comp_grid%lo(2),comp_grid%hi(2)
                do i = comp_grid%lo(1),comp_grid%hi(1)                    
                    v%x%f(i,j,k) = A*sin(comp_grid%z(k)) + C*cos(comp_grid%y(j))
                    v%y%f(i,j,k) = B*sin(comp_grid%x(i)) + A*cos(comp_grid%z(k))
                    v%z%f(i,j,k) = B*cos(comp_grid%x(i)) + C*sin(comp_grid%y(j))
                    p%f(i,j,k) = -(B*C*cos(comp_grid%x(i))*sin(comp_grid%y(j)) + &
                                   A*B*sin(comp_grid%x(i))*cos(comp_grid%z(k)) + &
                                   A*C*cos(comp_grid%y(j))*sin(comp_grid%z(k)))
                end do
            end do
        end do
