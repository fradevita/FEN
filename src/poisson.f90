module poisson_mod
    ...
    ! Fast Direct Poisson Solver. Available solvers, based on boundary conditions, are: 
    !     for 2D simulations:
    !         Periodic - Periodic (pp)
    !         Periodic - Neumann  (pn)
    !         Neumann  - Neumann  (nn)
    !     for 3D simulations:
    !         Periodic - Periodic - Periodic (ppp)
    !         Periodic - Periodic - Neumann  (ppn)
    !         Neumann  - Periodic - Neumann  (npn)
    !         Neumann  - Neumann  - Neumann  (nnn)
...
contains
    ...
    subroutine init_poisson_solver(phi)
        ...
        ! This subroutine select the proper Poisson Solver, call the init routine and assign
        ! the solve_poisson procedure pointer.
    ...
    !===============================================================================================
    subroutine init_poisson_pp(phi)
        
        ! Setup the variables for the Periodic-Periodic solver
    ...
    end subroutine init_poisson_pp
    !===============================================================================================
    
    !===============================================================================================
    subroutine init_poisson_pn(phi)
        
        ! Setup the variables for the Periodic-Neumann solver
    ...
    end subroutine init_poisson_pn
    !===============================================================================================
    
    !===============================================================================================
    subroutine init_poisson_nn(phi)
        
        ! Setup the variables for the Neumann-Neumann solver
...
#if DIM==3
    !===============================================================================================
    subroutine init_poisson_ppp(phi)
        
        ! Setup the variables for the Periodic-Periodic-Periodic solver
        ...
        ! Local variables
        integer  :: i, j, k, nx, ny, nz, lo(3), hi(3), lo_y(3), hi_y(3), lo_z(3), hi_z(3)
        real(dp) :: delta
    ...
    end subroutine init_poisson_ppp
    !===============================================================================================

    !===============================================================================================
    subroutine init_poisson_ppn(phi)
        
        ! Setup the variables for the Periodic-Periodic-Neumann solver
        ...
        ! Allocate memory for tridiagonal solver variables
        allocate( c1(lo_z(1):hi_z(1),lo_z(2):hi_z(2),lo_z(3):hi_z(3)))
        allocate(d1c(lo_z(1):hi_z(1),lo_z(2):hi_z(2),lo_z(3):hi_z(3)))
    
    end subroutine init_poisson_ppn
    !===============================================================================================

    !===============================================================================================
    subroutine init_poisson_npn(phi)
        
        ! Setup the variables for the Neumann-Periodic-Neumann solver
        ...
        ! Define FFT plans
        call dfftw_plan_dft_r2c_1d(pf_y, ny, outr_x_y(lo_y(1),lo_y(2):hi_y(2),lo_y(3)), &
                outc_y(lo_y(1),lo_y(2):hi_y(2),lo_y(3)), FFTW_ESTIMATE)
        call dfftw_plan_dft_c2r_1d(pb_y, ny, outc_y(lo_y(1),lo_y(2):hi_y(2),lo_y(3)), &
                outr_x_y(lo_y(1),lo_y(2):hi_y(2),lo_y(3)), FFTW_ESTIMATE)
    ...
    end subroutine init_poisson_npn
    !===============================================================================================

    !===============================================================================================
    subroutine init_poisson_nnn(phi)
        
        ! Setup the variables for the Neumann-Neumann-Neumann solver
        ...
        ! Allocate memory for the complex output of the DCT in the x direction
        allocate(outr_x(lo(1):hi(1),lo(2):hi(2),lo(3):hi(3)))
        ...
        call dfftw_plan_r2r_1d(pb_y, ny, outr_y(lo_y(1),lo_y(2):hi_y(2),lo_y(3)), &
                outr_y(lo_y(1),lo_y(2):hi_y(2),lo_y(3)), FFTW_REDFT01, FFTW_ESTIMATE)
    ...
    end subroutine init_poisson_nnn
    !===============================================================================================
    ...
    end subroutine poisson_solver_ppn
    !===============================================================================================

    !===============================================================================================
    subroutine poisson_solver_npn(phi)
        ...
        do k = lo_y(3),hi_y(3)
            do i = lo_y(1),hi_y(1)
                call dfftw_execute_dft_r2c(pf_y, outr_x_y(i,lo_y(2):hi_y(2),k), outc_y(i,lo_y(2):hi_y(2),k))
            end do
        end do

        ! Normalize output
        outc_y = outc_y / real(ny, dp)
        ...
        do k = lo_y(3),hi_y(3)
            do i = lo_y(1),hi_y(1)
                call dfftw_execute_dft_c2r(pb_y, outc_y(i,lo_y(2):hi_y(2),k), outr_x_y(i,lo_y(2):hi_y(2),k))
            end do
        end do
    ...
    subroutine destroy_Poisson_solver(phi)
        ...
        if (allocated(   mwn_x)) deallocate(   mwn_x)
        if (allocated(   mwn_y)) deallocate(   mwn_y)
        if (allocated(       a)) deallocate(       a)
        if (allocated(       b)) deallocate(       b)
        if (allocated(       c)) deallocate(       c)
        if (allocated(  outr_x)) deallocate(  outr_x)
        if (allocated(outr_x_y)) deallocate(outr_x_y)
        if (allocated(      c1)) deallocate(      c1)
        if (allocated(     d1r)) deallocate(     d1r)
        if (allocated(  outc_x)) deallocate(  outc_x)
        if (allocated(  outc_y)) deallocate(  outc_y)
        if (allocated(outc_x_y)) deallocate(outc_x_y)
        if (allocated(     d1c)) deallocate(     d1c)
#if DIM==3
        if (allocated(   mwn_z)) deallocate(   mwn_z)
        if (allocated(  outr_y)) deallocate(  outr_y)
        if (allocated(outr_y_z)) deallocate(outr_y_z)
        if (allocated(  outc_z)) deallocate(  outc_z)
        if (allocated(outc_y_z)) deallocate(outc_y_z)
#endif
