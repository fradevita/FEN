...
11    format(A99)
    ...
    function velocity(self, X, dir) result(v)
        ...
        d(2) = sqrt( (X(1) + base_grid%Lx - self%rot_center(1))**2 + (X(2) - self%rot_center(2))**2)
        d(3) = sqrt( (X(1) - base_grid%Lx - self%rot_center(1))**2 + (X(2) - self%rot_center(2))**2)
        i = minloc(d, 1)
        if (i == 1) then
            r = X - self%rot_center
        elseif (i == 2) then
            r = X + [base_grid%Lx, 0.0_dp, 0.0_dp] - self%rot_center
        elseif (i == 3) then
            r = X - [base_grid%Lx, 0.0_dp, 0.0_dp] - self%rot_center
        endif
...
contains
    ...
    function distance(self, X) result(d)
        ...
        if (base_grid%periodic_bc(1)) then
            dd(1) = d
            dd(2) = sqrt((X(1) - self%X(1) + base_grid%Lx)**2 + (X(2) - self%X(2))**2) - self%R
            dd(3) = sqrt((X(1) - self%X(1) - base_grid%Lx)**2 + (X(2) - self%X(2))**2) - self%R
            d = dd(minloc(abs(dd), 1))
        endif
        if (base_grid%boundary_conditions(3)%s == 'Periodic') then
            dd(1) = sqrt((X(1) - self%X(1))**2 + (X(2) - self%X(2) + base_grid%Ly)**2) - self%R
            dd(2) = sqrt((X(1) - self%X(1))**2 + (X(2) - self%X(2) - base_grid%Ly)**2) - self%R
            d = min(d, dd(1), dd(2))
        endif
