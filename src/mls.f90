...
contains
    ...
    function interpolate(f, xl, ie, ind) result(fl)
        ...
        ! In/Out variables
        integer     , intent(in) :: ie(2), ind
        real(dp)    , intent(in) :: xl(Ndim)
        type(scalar), intent(in) :: f
        
        ! Local variables
        integer  :: si, sj, q, ii, jj, ierror
        real(dp) :: xe, ye, xs(Ndim,Ne), fl(m), ds(Ndim,Ne), fk(Ne), stagger(2)
        ...
        xe = base_grid%x(ie(1)) + stagger(1)
        ye = base_grid%y(ie(2)) + stagger(2)

        ! Select the rank containing the Eulerian point
        rank_with_point: if (ie(2) >= base_grid%lo(2) .and. ie(2) <= base_grid%hi(2)) then 
            ...
            do sj = -1,1
                ...
                do si = -1,1
                    ii = ie(1) + si
                    xs(1,q) = xe + si*base_grid%delta
                    xs(2,q) = ye + sj*base_grid%delta
                    fk(q) = f%f(ii,jj,1)
                    ds(1,q) = merge(base_grid%delta, 2.0_dp*base_grid%delta, si == 0)
                    ds(2,q) = merge(base_grid%delta, 2.0_dp*base_grid%delta, sj == 0)
                    q = q + 1
                end do
            end do
        
            ! Size of the support domain
            ds = 1.60_dp*base_grid%delta
